...
impl AgentService {
    ...
    pub async fn list(&self) -> Result<Vec<Agent>> {
        ...
        let agent_configs = config.agents.and_then(|a| a.list).unwrap_or_default();
        ...
        let agents = agent_configs
            ...
            .map(|ac| Agent {
                ...
                model: ac
                    .model
                    .map(|m| crate::models::agent::AgentModel { primary: m.primary }),
            })
            .collect();
    ...
    pub async fn read_files(&self, agent_id: &str) -> Result<AgentFiles> {
        ...
        let system = self
            .read_file_optional(&agent_dir.join("system.md"))
            .await?;
        ...
        let models = self
            .read_file_optional(&agent_dir.join("models.json"))
            .await?;
        let auth_profiles = self
            .read_file_optional(&agent_dir.join("auth-profiles.json"))
            .await?;
    ...
    pub async fn delete(&self, agent_id: &str) -> Result<()> {
        let agent_dir = self.openclaw_home.join("agents").join(agent_id);
