...
export default function SkillsTab() {
    ...
    const loadSkills = useCallback(async () => {
        ...
        try {
            ...
            if (currentAgent?.isMain) {
                // 主 Agent - 使用默认方法
                list = await skillApi.listLocal()
            } else if (currentAgent) {
                // 其他 Agent - 从特定工作区加载
                list = await skillApi.listAgentSkills(currentAgent.skillsPath)
            } else {
                list = []
            }
            ...
            // Set directory path
            if (currentAgent) {
                setSkillsDir(currentAgent.skillsPath)
            }
        } catch (err) {
            setError(String(err))
        } finally {
    ...
    const renderSubTabContent = () => {
        switch (activeSubTab) {
            ...
            case 'builtin':
                return renderBuiltinTab()
            case 'clawhub':
                return renderClawHubTab()
            case 'ai':
                return renderAiTab()
            case 'identity':
